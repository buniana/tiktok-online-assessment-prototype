...
**Full preview:** Finish opens a preview of the whole video: every shot's take played back to back, looping. The back arrow returns to the last review; Next returns to the start. Layout: the designer's edit-screen screenshot, see section 18.
...
## 16. Back button and PLAN header on every chat screen (2026-09-20)

- **Header on all chat screens:** the back arrow and the centred "PLAN" title now show on the voice screens (listening, AI question, tap-to-speak, "Got it") as well as on the plan screen, following the designer's mockup (same position as on the plan screen: title centred at y = 80, arrow centred at (34, 80)). They come in once the chat has slid up. The hairline under the header stays plan-only (it fades in from "Thinking" onward). After the first message lands, the title sits just under the header (top 107 vs header bottom 112); adjust if it feels tight.
- **Back exits the chat** to the first screen (the camera / filming screen with PLAN): the chat slides down (0.46s, `cubic-bezier(.4,0,.2,1)`, a touch quicker than it came in) while the camera screen scales back from 0.92 to 1 at the same time, then the whole conversation is thrown away, so the next tap on PLAN starts a fresh chat. It also cancels any spoken audio and releases the real microphone (the mic is requested again the next time the chat opens, only when simulated sound is off).
- **Bug fixed along the way:** the back arrow never received taps, because the full-width "PLAN" title box sat on top of it; the title and hairline are now click-through. Verified with a mouse and with real touch taps on a phone-sized screen, from a voice screen and from the plan screen.
- The mockup shows the label lowercase ("listening…"); the app still says "Listening…" (capital L, as agreed earlier).

## 17. Toolbar icons redrawn (2026-09-20, awaiting the designer's confirmation before they are exported)

- **Where:** `src/components/CamTools.jsx` (shared by the camera screen and the filming screen). Each icon is drawn in a 32 x 32 dp box centred on where the original sits, in white, with plain shapes only (no blur or filters).
- **How:** traced from the original camera frame at half brightness (sub-pixel), then fitted with circles and straight edges; sizes are the original's (width x height, dp): flip 28.4 x 24.1, flash 22.6 x 26.0, effects 27.3 x 25.5, timer 25.9 x 25.9, layout 23.5 x 23.6, beauty 25.5 x 24.8 (original 28.4 x 24.5, 22.7 x 26.1, 27.4 x 25.6, 25.8 x 25.8, 23.5 x 23.6, 25.2 x 24.9). Overlap with the traced originals: timer 95%, layout 96%, effects 91%, flash 90%, beauty 87%.
- **Flip:** follows the designer's zoomed reference: two constant-width arcs (2.56 wide, radius 10.75, each about 110 degrees) ending in small rounded arrowheads (7.3 wide, 5.6 tall; they were about 10 x 8 before), the lower half the upper turned 180 degrees about (16.4, 16.6).
- **Flash:** a six-point bolt cut by a diagonal 2.4-wide slash with a 2.2 dp gap on its lower-left side.
- **Timer:** the ring, the centre dot and the needle now share one centre (15.97, 15.65); the needle points 45 degrees up-left, to the middle of the gap. (Before, the dot and needle sat about 3 dp below and right of the dial's centre.)
- **Effects:** now has the second ring behind the face (the crescent at the lower right) that the original has.
- **Beauty:** the pink badge is 12.9 across (it was drawn about a third too small).
- Not exported yet: SVG files (one per icon, plus the whole toolbar) come after the designer confirms.

## 18. Full-video preview layout, bottom buttons, tap hand, refresh (2026-09-21)

Source for the layout and the button numbers: the designer's screenshot `IMG_6966.PNG` (in their Downloads folder; 1320 x 2868 = exactly 3x the 440 x 956 design frame, so every number below is image px / 3).

- **Bottom buttons (measured, sub-pixel edges from the anti-aliasing):** white and pink pills are both **204 x 44 dp**, full pill (radius 22), **8 apart**, **12 from each side edge**, **top 874 / bottom 918 (38 above the screen bottom)**, 18 below the video panel (panel bottom 856). Pink is `#FE2C55` (254, 44, 85). Applied to the buttons on the segment previews (`.fa`, `.fa-retake` left 12, `.fa-next` left 224); Finish now has the same geometry as Next segment (it used to be a slightly different size). Labels on the segment previews are unchanged (600 16px).
- **Full-video preview screen (`Preview` in `FilmFlow.jsx`, icons in `src/components/PreviewIcons.jsx`):** the video panel is the same as on the filming screens (top 74, height 782, radius 24; the screenshot's panel is 73.7 to 856) with the stock clips playing in it. On top: back arrow (11.7 x 20 at centre 27.5, 109.7), "Add sound" pill (155, 88, 130 x 43.7; note traced at 172.3, text 600 16px starting 189.3), the tool column at x centre 414 (settings 109.7, share 161.5, thin separator 404-424 at 194.2, layout 226.7, play cards 278.8, Aa 330.3, sticker 383, effects sparkle 434.7, filters 486.7, voice 538.7, chevron 586.2), the AutoCut pill (172.3, 797, 95.3 x 36; label 600 13px), and the bottom row: "Your Story" (white, gradient-ring avatar 33 dp at 40 from the pill's left edge, purple disc `#AC47BB` with a white L, label starts ~4 after the avatar, `#2D2E38`) and "Next" (pink). The two bottom labels are 600 at 16.5px (fitted to the ink widths 81.7 and 35.3 in the screenshot). Pills are grey 61 at 50% (fit to the fill over the dark panel).
- **Icons are traced, not hand-drawn:** each outline was extracted from the screenshot at the 50% anti-aliasing level (marching squares) and simplified to within 0.04 dp; overlap with the screenshot at the sharp edge is 97 to 99.9% per icon. The screenshot's edges are slightly soft (a thin halo), which is not reproduced.
- **What the screen does:** the back arrow returns to the last segment's review; **Next** returns to the start; **Your Story** and the tool column are drawn only (nothing after "Next" exists yet). The old segmented progress bar along the top is gone (the screenshot has none).
- **Verified against the screenshot** (headless Chrome at 3x, dark panel instead of video): button boxes identical; label ink boxes within 0.3 to 0.7 dp (text snaps to whole pixels vertically); "Add sound" exact; avatar ring colours within 1 to 3 levels.
- **Tap hand on the camera screen:** now the designer's `Frame.svg` (drawn 86 x 74, tilt included), shown at **85%** (73.1 x 62.9) so it no longer covers "PLAN". Its fingertip is at (305.75, 904): the same column as the old hand's fingertip and 6.5 lower, which puts the top of the tap ripple 1.7 below the PLAN text (text ink 270 to 307.3, 882.7 to 893.3), also in the pressed pose of the tap animation. Same tap motion and timing as before (translate -2 / -3, scale 0.92, 1.8s); the old extra -16 degree rotation is gone because the tilt is in the drawing. The hand is outline only (the old one had a black fill).
- **Refresh restarts the flow:** the URL shortcuts that choose a start screen (`jump`, `open`, `ver`, `fseg`, `fphase`, `fp`) are used once and then removed from the address bar (`history.replaceState`), so a browser refresh starts from the camera screen. Reason: a page opened with `?open=1` or `?jump=...` replayed that shortcut on every refresh. The flags `debug`, `fit`, `sim`, `copy` stay. Nothing else is stored between visits except the simulated-sound setting.
